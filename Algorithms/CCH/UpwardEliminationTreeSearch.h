...
public:
    ...
    UpwardEliminationTreeSearch(
            const SearchGraph& searchGraph, const std::vector<int32_t> &eliminationTree,
            PruningCriterionT pruneSearch = {})
            : UpwardEliminationTreeSearch(searchGraph, &searchGraph.template get<CH::Weight>(0), eliminationTree,
                                          pruneSearch) {}
...
private:
    ...
    const GraphT &searchGraph;          // The upward or downward graph we work on.
