 ...
 */
    class Graph {
        ...
        static void
        extend_on_partition(std::vector<CutIndex> &ci, double balance, uint8_t cut_level, const std::vector<NodeID> &p,
                            const std::vector<NodeID> &cut, size_t leaf_size);
    ...
    public:
        ...
        void get_anc_dist(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t h_max,
                          std::vector<Neighbor> &result) const;//, size_t &trunc_anc, size_t &untrunc_anc);
        void get_anc_dist(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t h_max,
                          std::vector<QNode> &result) const;

        distance_t query_distance(ContractionHierarchy &ch, NodeID s, NodeID t);

        distance_t query_distance(ContractionHierarchy &ch, ContractionIndex &tcl, NodeID s,
                                  NodeID t);//, size_t &covered_pairs, size_t &trunc_anc, size_t &untrunc_anc);
        std::vector<NodeID> query_path(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, NodeID w,
                                       distance_t &dist) const;

        void get_anc_dist(const ContractionHierarchy &ch, NodeID v, std::vector<QNode> &result) const;
        ...
        void path_from_anc(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t w,
                           std::vector<QNode> &anc_dist, std::vector<NodeID> &result) const;

        void path_from_anc(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, NodeID w,
                           std::vector<QNode> &anc_dist, std::vector<NodeID> &result) const;

        void
        unpack_valley_path(const ContractionHierarchy &ch, NodeID v, uint16_t w, std::vector<NodeID> &result) const;
        ...
        void unpack_convex_path(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t w,
                                std::vector<NodeID> &result) const;

        void unpack(CHNeighbor const &n, std::vector<NodeID> &path, bool reverse);
        ...
        void random_pairs(std::vector<std::vector<std::pair<NodeID, NodeID>>> &buckets, distance_t min_dist,
                          size_t bucket_size, const ContractionIndex &ci);
