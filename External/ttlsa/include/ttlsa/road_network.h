...
public:
    ...
    // start of path labels for given cut level
    const CHNeighbor* cl_begin_paths(size_t cl) const;
    // end of path labels for given cut level
    const CHNeighbor* cl_end_paths(size_t cl) const;


    // returns labels in list-of-list format
    std::vector<std::vector<distance_t>> unflatten() const;
    std::vector<std::vector<CHNeighbor>> unflatten_paths() const;
...
struct composite_shortcut
{
    NodeID _no_node, triangle_node;
    CHNeighbor *lower, *upper;
    composite_shortcut();
};
...
struct CHNeighbor
{
    ...
    distance_t distance;
    path_data p;
    CHNeighbor(NodeID neighbor, distance_t distance, path_data p);
};

struct QNode
{
    CHNeighbor shortcut_used;
    uint16_t shortcut_start;
    NodeID node_id;
    distance_t distance;
    QNode(CHNeighbor shortcut_used, uint16_t shortcut_start, NodeID node_id, distance_t distance);
};
...
public:
    ...
    void get_anc_dist(const ContractionHierarchy &ch, NodeID v, std::vector<CHNeighbor> &result) const;
    void get_anc_dist(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t h_max, std::vector<Neighbor> &result) const;//, size_t &trunc_anc, size_t &untrunc_anc);
    void get_anc_dist(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t h_max, std::vector<QNode> &result) const;
    distance_t query_distance(const ContractionHierarchy &ch, NodeID s, NodeID t) const;
    distance_t query_distance(const ContractionHierarchy &ch, ContractionIndex &tcl, NodeID s, NodeID t) const;//, size_t &covered_pairs, size_t &trunc_anc, size_t &untrunc_anc);
    std::vector<NodeID> query_path(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, NodeID w, distance_t& dist) const;

    void find_shortcut(const ContractionHierarchy &ch, NodeID prev, uint16_t current, CHNeighbor &n) const;
    void find_shortcut(const ContractionHierarchy &ch, NodeID prev, NodeID current, CHNeighbor &n) const;
    void path_to_anc(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t w, std::vector<QNode> &anc_dist, std::vector<NodeID> &result) const;
    void path_from_anc(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t w, std::vector<QNode> &anc_dist, std::vector<NodeID> &result) const;
    void unpack_valley_path(const ContractionHierarchy &ch, NodeID v, uint16_t w, std::vector<NodeID> &result) const;
    void unpack_valley_path(const ContractionHierarchy &ch, NodeID v, NodeID w, std::vector<NodeID> &result) const;
    void unpack_convex_path(const ContractionIndex &tcl, NodeID v, uint16_t w, std::vector<NodeID> &result) const;
    void unpack_convex_path(const ContractionHierarchy &ch, const ContractionIndex &tcl, NodeID v, uint16_t w, std::vector<NodeID> &result) const;

    void unpack(CHNeighbor const& n, std::vector<NodeID> &path, bool reverse) const;
    ...
    distance_t cutDist(distance_t d);
