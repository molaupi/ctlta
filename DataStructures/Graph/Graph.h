...
public:
    ...
    uint64_t sizeInBytes() const {
        ...
        size += outEdges.size() * sizeof(OutEdgeRange);
        size += edgeHeads.size() * sizeof(int32_t);
        RUN_FORALL(size += VertexAttributes::values.size() * sizeof(typename VertexAttributes::Type));
        RUN_FORALL(size += EdgeAttributes::values.size() * sizeof(typename EdgeAttributes::Type));
        return size;
    }
